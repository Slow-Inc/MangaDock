---
name: feedback_clear_cache_before_test
...
**สำคัญ — หลัง deploy โค้ด/fix:** L3 เก็บ **response เก่า** → replay ของก่อน fix (เช่น patch url ที่ยังไม่มี `?v=` cache-bust ของ patch-store) ทำให้ fix "เหมือนไม่ติด". ต้องล้าง **ครบทั้ง 3 ชั้น (รวม L3) + restart backend + reload browser** เพราะ frontend เก็บ `patchedPages` ใน memory → "แปลหน้านี้" จะ short-circuit คืน patch เก่าไม่เรียก backend (หรือ test บนหน้าที่ยังไม่เคยแปลใน session). เคส #170: ลืมล้าง L3 หลัง fix → browser โชว์ strip เก่า natH=1492 (จับได้ด้วยเทียบ naturalHeight เก่า/ใหม่). เชื่อมกับ [[feedback_test_every_round]]
