---
name: project-benchmark-e2e-flow
tags: ["project"]
...
**Full stack:** restart MIT worker บนโค้ดใหม่ (kill 5003+5004 by port owner ตาม [[project-mit-worker-restart-gotcha]]) → Backend :4001 → Frontend :4000 → cloudflared tunnel (`cloudflared tunnel run`, config `~/.cloudflared/config.yml`) → domain **`hayateotsu.space`** (→:4000), `api.hayateotsu.space` (→:4001).
