...
tags: ["feedback"]
description: Tech-debt PR policy — auto-merge when green, but REVIEW FIRST with /scrutinize AND rebase onto latest main if the base went stale before merging (the #553 clobber lesson)
metadata:
  type: feedback
---
...
5. **Before merging, verify the base is not stale.** `git fetch origin main`, then check whether main advanced past the PR's branch point (`git merge-base --is-ancestor origin/main HEAD` → non-zero exit = main moved; or `git log --oneline origin/main ^HEAD`). If it did, **`git rebase origin/main`, resolve, then re-run the full suite AND re-run `/scrutinize` (re-stamp the marker — HEAD moved)** before merging. A PR merged on a stale base can silently revert work that landed on main after the branch point — the **#553 stale-base clobber** (→ #608, Stage C wiring shipped OFF for weeks). This is the human half of the append-only guard ([[feedback-review-merge-policy]] pairs with `scripts/check-append-only.mjs`, #611) + the "Require branches to be up to date before merging" branch-protection setting; do it even for a `gh`/CLI merge where the GitHub setting can't reach.
6. If clean → squash-merge → sync main → delete branch → notify the dev.
7. Never merge if `/scrutinize` finds a real blocker; surface it instead.
