...
**Don't debug MIT pipeline internals through the worker HTTP loop** when you need stage-by-stage visibility — the restart+log indirection is too flaky. Run the pipeline **in-process** (kill the worker to free the GPU first, reliably via port-owner) so `logger.info` prints straight to stdout. See [[project-mit-launch-env]].
