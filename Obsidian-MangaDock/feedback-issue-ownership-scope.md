---
name: feedback-issue-ownership-scope
tags: ["feedback"]
...
**Why:** the team shares one issue tracker but each owner files their own backlog. Implementing someone else's logged issue steps on their plan and crosses the area split ([[project-team-split]]). Confirmed 2026-06-11 by the dev.
...
- Batch related OURS issues into one **epic-PR per category** (1 issue = 1+ commit, test each step, full suite + E2E before opening the PR), capped at ~2-4 issues/PR; never mix a decision/design issue into a do-now batch. See [[feedback-decomposition-method]].
