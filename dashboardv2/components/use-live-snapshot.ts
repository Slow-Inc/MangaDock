...
import { MOCK_MIT, MOCK_SERIES, MOCK_EVENTS } from "@/lib/mock-live";
...
export interface Live {
  ...
  error?: string;
  mock?: boolean; // mock-mode snapshot (NEXT_PUBLIC_MOCKUP_MODE) — for the "MOCKUP DATA" badge
}

// A representative live snapshot for mock mode — same shape the real stream produces, so every
// panel renders it identically and the mock→real switch can't strand a panel (lib/mock-live).
function buildMockLive(): Live {
  const now = Date.now();
  const seriesT = Array.from({ length: 30 }, (_, i) => now - (29 - i) * 15000);
  const events = MOCK_EVENTS.map((e) => ({ type: "event", kind: e.kind, detail: e.detail, at: now - e.agoS * 1000 })) as unknown as Message[];
  return { status: "live", mit: MOCK_MIT, events, series: MOCK_SERIES, seriesT, mock: true };
}
...
export function useLiveSnapshot(token: string | null): Live {
  const [live, setLive] = useState<Live>(() => (isMockMode() ? buildMockLive() : { status: "connecting", mit: null, events: [], series: {}, seriesT: [] }));
  const stateRef = useRef<State>(initialState());
  const seriesRef = useRef<SeriesMap>({});
  const seriesTRef = useRef<number[]>([]);

  useEffect(() => {
    // Mock mode (NEXT_PUBLIC_MOCKUP_MODE): serve a representative snapshot, never touch the network.
    if (isMockMode()) {
      setLive(buildMockLive());
      return;
    }
    if (!token) {
