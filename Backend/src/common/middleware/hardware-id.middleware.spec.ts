import { HardwareIdMiddleware } from './hardware-id.middleware';
...
describe('HardwareIdMiddleware', () => {
  ...
  ])('should pass through %s without HWID (guarded by AuthGuard, not HWID)', (path) => {
  ...
  it.each([
    ...
    expect(mockResponse.json).toHaveBeenCalledWith(
      expect.objectContaining({ message: 'Missing hardware ID' }),
    );
  ...
  it.each([
    ...
    '/versions/v1',
    '/upload/something',
  ])('should allow %s when HWID header is present and attach it to request', (path) => {
    mockRequest.path = path;
