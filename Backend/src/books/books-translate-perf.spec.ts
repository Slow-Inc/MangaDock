...
describe('BooksService — parallel cache round-trips in translate paths (#148)', () => {
  ...
  it('batch pre-check issues every page-cache get before any resolves', async () => {
    ...
    const pages = [0, 1, 2].map((i) => ({
      pageIndex: i,
      pageUrl: `http://img/${i}.jpg`,
    }));
    const received: number[] = [];

    const job = service.startOrAttachBatchJob(
      'ch1',
      pages,
      (pageIndex) => {
  ...
  it('text translate issues every line-cache lookup before any resolves, and counts hits', async () => {
    ...
    const call = service.translateMangaEpisode({
      lines,
      chapterId: 'ch1',
      page: 0,
      targetLang: 'th',
    ...
    for (const d of deferredByKey.values())
      d.resolve({ data: { text: 'แปลแล้ว' }, source: 'json' });
