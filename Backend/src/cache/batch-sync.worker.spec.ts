...
function makeClient(dirtyQueue: string[], store: Record<string, string> = {}, processingQueue: string[] = [], hincrbyResult = 1) {
  return {
    ...
    hincrby: jest.fn().mockResolvedValue(hincrbyResult),
...
describe('BatchSyncWorker — Reliable Queue', () => {
  ...
  describe('retry budget & dead-letter (#65 / #66)', () => {
    // R1 — Tracer bullet: HINCRBY on RPC failure
    ...
    it('cleans up retry counter with HDEL when key has expired in L2', async () => {
      const { worker, redis } = makeWorker(['ghost-key'], {});
