...
export class L2RecoveryService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.election.onBecomeLeader(() => {
      this.recover().catch((err) =>
        this.logger.warn(`L2 recovery failed: ${String(err)}`),
      );
    });
  ...
  async recover(): Promise<{ synced: number; skipped: number }> {
    ...
    if (allKeys.size === 0 && fallbackKeys.length === 0)
    ...
    for (let i = 0; i < toSync.length; i += PIPELINE_CHUNK_SIZE) {
      ...
      try {
        ...
        this.logger.warn(
          `L2 recovery: chunk [${i}–${i + chunk.length - 1}] failed: ${String(err)}`,
        );
      }
    }
  ...
  private newerEntry(
    l1?: CacheEntry<unknown>,
    l3?: CacheEntry<unknown>,
  ): CacheEntry<unknown> {
    if (!l1) return l3!;
    if (!l3) return l1;
