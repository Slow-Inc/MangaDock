...
function makeRedis(
  ...
  Pick<
    RedisService,
    'available' | 'get' | 'set' | 'publish' | 'subscribe' | 'onReconnect'
  >
> {
  return {
    available,
...
function makeJsonCache(): jest.Mocked<
  Pick<JsonCacheService, 'get' | 'set' | 'delete' | 'clear' | 'isExpired'>
> {
  return {
...
function makeL3(): jest.Mocked<
  Pick<L3DiskService, 'appendDirtyFallback' | 'drainDirtyFallback'>
> {
  return {
...
describe('CacheOrchestratorService — cross-node L1 invalidation (#37)', () => {
  ...
  it('set() publishes { key, nodeId } to cache:invalidate after writing to L2', async () => {
    ...
    const { svc, redis } = makeOrchestrator({
  ...
  it('onReconnect clears L1 (jsonCache.clear) before re-subscribing to invalidation channel', () => {
    ...
    (jsonCache.clear as jest.Mock).mockImplementation(() =>
      callOrder.push('clear'),
    );
    (redis.subscribe as jest.Mock).mockImplementation(() =>
  ...
  it('onModuleInit() subscribes to cache:invalidate; message from another node deletes key from L1', () => {
    ...
    (redis.subscribe as jest.Mock).mockImplementation(
      (_ch: string, handler: (data: unknown) => void) => {
  ...
  it('onModuleInit() handler deletes from L1 when incoming nodeId differs from own nodeId', () => {
    ...
    svc.onModuleInit();
    capturedHandler!(
      JSON.stringify({ key: 'manga:1', nodeId: 'another-node' }),
    );
...
describe('CacheOrchestratorService — L1-first read path (#36)', () => {
  ...
  it('get() writes the L2 entry into L1 on a L2 cache hit', async () => {
    ...
    expect(jsonCache.set).toHaveBeenCalledWith(
      'manga:1',
      entry.data,
      expect.any(Number),
    );
