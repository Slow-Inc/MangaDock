...
export default function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  const socialResolvedRef = useRef(false);
  ...
  /** Wraps a social sign-in call with popup-close detection via window-focus. */
  const signInWithPopupGuard = (provider: "google" | "facebook", fn: () => Promise<void>) => async () => {
    setAuthError(null);
    setSocialLoading(provider);
    socialResolvedRef.current = false;

    let focusTimer: ReturnType<typeof setTimeout> | null = null;
    const onFocus = () => {
      focusTimer = setTimeout(() => {
        if (!socialResolvedRef.current) setSocialLoading(null);
      }, 2000);
    };
    ...
    try {
      await fn();
      socialResolvedRef.current = true;
      handleClose();
    } catch (e: unknown) {
      socialResolvedRef.current = true;
      const code = (e as { code?: string })?.code ?? "";
      if (code === "auth/popup-closed-by-user" || code === "auth/cancelled-popup-request") {
        // User closed the popup — silent, no error message
      } else {
