...
| `__init__.py` | **#359 lazy package boundary:** replaced eager `from .manga_translator import *` with PEP 562 `__getattr__`/`__dir__` forwarding (via `importlib.import_module` to avoid self-name recursion). `import manga_translator` + lightweight submodule imports are torch-free; `from manga_translator import Config/Context/MangaTranslator/...` resolves on first access. ADR 023 | #359 — torch-free logic tests / blocking CI gate | Re-add eager `import *` → every import pulls torch+cv2+transformers+diffusers (multi-GB) again; logic CI gate dies |
| `utils/__init__.py` | **#359:** stop eager `from .inference import *` (the only torch puller in utils); forward its names (`ModelWrapper`/`InfererModule`/...) via PEP 562 `__getattr__` (importlib, self-name-safe). Other 6 star-imports (sort/generic/textblock/...) stay eager (cv2/numpy, cheap) | #359 | Re-add eager inference import → every `from manga_translator.utils import X` pulls torch |
...
|------|----------------|-----|
| `instance.py` | removed unused batch send methods; lock released before waiting in `find_executor`; `sent_patches` threads `progress_meta` to the worker; **2026-06-16** `ExecutorInstance` gains `pid`/`registered_at`, `Executors.detail_raws()` → `workers.detail[]` (Dev-console worker lifecycle) | #106 contention; live progress UX; #279 |
| `main.py` | `/ready` (probes worker `/health`); batch endpoint gained webhook fire-and-forget mode (202 + `run_batch_with_callbacks`); `/cancel/{taskId}`; `safe_result_folder` path guard; worker always binds 127.0.0.1; removed legacy batch endpoints; **#193** `--start-instance` worker lifecycle — startup port-collision pre-check (fail loud, not hang) + `atexit` & `__main__` finally orphan cleanup (uvicorn overrides our signal handlers) + front/worker PID logging, delegating to `worker_lifecycle.py`; **#279/ADR 019** Dev console: `GET /status` + `GET /status/stream` (SSE) behind a `require_staff` dep (forward-JWT → `auth.verify_supabase_token` + `is_staff`; stream re-validates every `MIT_STATUS_REVALIDATE_S` + closes on expiry); throttled gateway probe (`MIT_DIAG_INTERVAL_S`); `/register` pushes a `worker registered` event — **parent server only**, never the worker; **2026-06-16** `POST /internal/telemetry` (`X-Nonce`) ingests the worker's stage-timing + VRAM telemetry; `_collect_snapshot` folds `queue.jobs` / `workers.detail` / `telemetry_store.snapshot()` (stages+vram) in | #100 #101 #102 #103 #106 + 2026-06-06 incident + #193 + #279 |
| `myqueue.py` | single-image tasks only (no disk offload, no `BatchQueueElement`); `QueueElement.progress_meta`; **#279** `TaskQueue.add_task` pushes a `translate_triggered` event to `status_hub` (one enqueue chokepoint); **2026-06-16** `QueueElement` gains an id + enqueue time, `jobs_snapshot()` → `queue.jobs[]` via `queue_view` | batch moved to `batch_runner`; live progress UX; Dev-console event push + queue panel |
...
`diagnostics.py` (**Staff Console #279 / 1b**, ADR 018 — cheap bounded translator-gateway probe, decoupled from the worker pool, classifies `ok/slow/timeout/auth/unreachable/model_missing` and distinguishes gateway-up vs model-down via `/models`; the 2026-06-14 incident reads as `timeout` "gateway /models OK but chat completion timed out — model not responding"; **2026-06-16** `GatewayStatus.control_ms` times the `/models` control-plane call separately from the chat data-plane `latency_ms`, so the Dashboard renders a real control-vs-data split (control up + data timeout = model hung); import-light httpx-only, `test/test_diagnostics.py` ×7) ·
...
**Dev console — MIT live telemetry (#279 / 1d+1f, ADR 018 + ADR 019), 7 new import-light modules (TDD <1s):**
- `status_snapshot.py` — `build_snapshot(...)` folds metrics+diagnostics+queue/workers/translator into the wire shape `Dashboard/lib/snapshot.ts` consumes (overall status up/degraded/down); `to_messages` emits the SSE frames (the `metric` frame carries the **whole** snapshot). `_gateway_dict` forwards the gateway `control_ms` (2026-06-16) alongside `latency_ms`. Pure/stdlib (gateway duck-typed). `test/test_status_snapshot.py` ×6.
- `auth.py` — `verify_supabase_token` via Supabase `GET /auth/v1/user` (httpx; **no PyJWT / no JWT secret in MIT**, mirrors the Backend's `getUser`; **refines ADR 018**'s PyJWT assumption — robust to the new asymmetric `sb_publishable_…` keys) + `is_staff` (staffLevel claim OR `MIT_STAFF_USER_IDS` allowlist). Fails closed. `test/test_auth.py` ×8.
- `status_hub.py` — `StatusHub` asyncio pub/sub; non-blocking `put_nowait` fan-out so the translate path can push an event without awaiting and a stuck consumer drops (never blocks). Module singleton `status_hub`. `test/test_status_hub.py` ×5.
- `status_stream.py` — `format_sse` (compact, `ensure_ascii=False` for TH/JP) + `status_frames` hybrid generator (initial sample → push events via `asyncio.wait_for`, sample on the interval timeout; awaits a coroutine sampler so the route can re-validate the token). `test/test_status_stream.py` ×5.
- `telemetry_store.py` — **(2026-06-16)** parent-side sink the worker reports pipeline telemetry into; `apply()` dispatches `stage` (rolling per-stage durations) / `vram` (the worker's torch alloc/reserved + per-model footprint & leak flags) and returns a `status_hub` event for notable ones. Folded into the snapshot's `stages`/`vram`. `test/test_telemetry_store.py` ×8.
- `queue_view.py` — **(2026-06-16)** pure transform: TaskQueue contents → `queue.jobs[]` (id/task_type/page/waiting_ms — only what the patch path carries; no user/manga invented). `test/test_queue_view.py` ×4.
- `worker_view.py` — **(2026-06-16)** pure transform: executor registry → `workers.detail[]` (ip/port/pid/uptime/busy). `test/test_worker_view.py` ×3.

**Worker → parent telemetry channel (2026-06-16):** `main.py` `POST /internal/telemetry` (`X-Nonce` auth, same as `/register`) receives the worker's stage timings + per-page VRAM report → `telemetry_store` → snapshot (+ a `status_hub` event when notable). `webhook.py` gained `make_telemetry_hook` (times each stage = gap between consecutive stage starts, posts to the parent) + `send_telemetry`; the worker attaches the hook + posts `vram_report()` per page in `mode/share.py` (which also stores the parent url/nonce in `_PARENT` at startup). **Per-model VRAM leak monitoring:** `manga_translator/vram_probe.py` reads the worker's `torch.cuda.memory_allocated/reserved` (the one torch-touching spot; None without CUDA); `manga_translator/vram_tracker.py` `VramTracker` learns each model slot's footprint from its largest clean release and flags an unload that frees `< leak_ratio` of it (`leaked`). Fed by `model_unloader.ModelUnloader` (the #188 unload seam, now with injected `read_vram`+`vram_tracker`) — **no load-site instrumentation** (touch sites are pre-load; post-load deltas are activation-noisy → footprint is learned from unloads). `MangaTranslator.vram_report()` = `{allocated_mb, reserved_mb, models:[…]}`. `test/test_vram_tracker.py` ×5, `test/test_model_unloader.py` +2.

**Revert hazard (dev console):** these are additive seams — removing them drops `GET /status` + `/status/stream` (the Dashboard falls back to mock); they touch no translate path. The VRAM/telemetry channel is worker-reported and fire-and-forget — dropping it never disturbs translation.
