...
- `translator_chain.py` (#192a) — `TranslatorChain` parse. · `config.py::parse_and_validate_config` (#192) — the single config parse/validate seam (Pydantic-v2 `model_validate_json`) every endpoint shares, replacing 11× scattered `Config.parse_raw` (load_dotenv extraction deferred: import-order risk > ROI; the remaining bare-excepts are intentional broad catches, not debt). · `line_break.py` (#180) — Knuth-Plass packer behind the **#186 `LineBreaker` seam** (`KnuthPlassLineBreaker`). **#180 step 2 DONE**: `dispatch()` now selects KP for the rendered glyphs (`render` → `put_text_horizontal` → `calc_horizontal`, new `line_breaker=` param) when `render.bubble_area_fit` is on → balanced bubble-filling wrap; off → byte-identical greedy. Benchmark-verified (`tools/ab_benchmark.py`, JA→EN). Pure additions (sibling `kinsoku.py`, moved to package root to stay PIL-free): `respect_kinsoku` (large-but-finite 行頭/行末禁則 penalty) + `hyphenate_overwide_tokens` (Latin over-wide word split). Follow-ups: activate `respect_kinsoku` for CJK targets; mid-word hyphenation through the seam's syllable contract.
