...
- `render_overlap.py` (anti-overlap text layout) — pure stdlib geometry (no ML): `apply_font_cap(size, cap, is_sfx)` caps a non-SFX render font so narration/caption can't be length-ratio-scaled into an oversized block (with `RenderConfig.font_size_max`; the renderer also pins `final_scale=1.0` for non-SFX so the long translation wraps inside the source box instead of overflowing — SFX exempt). `clamp_box_to_neighbors(box, others, margin)` shrinks a region's render box so the translated text can't grow into a neighbour's territory (separate along the axis of least penetration, pull only the facing edge). Wired into `rendering/__init__.py` both paths — bubble-fit (clamp the fit box) and the fallthrough (clamp the warped `dst_points` so the homography shrinks the text). Gated by `RenderConfig.anti_overlap`; off → byte-identical. `test/test_render_overlap.py`. Addresses the user-flagged text overlap vs the MangaTranslator target (with `bubble_area_fit` off + `anime_ace_3` for the small/light weight).
