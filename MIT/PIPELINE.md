...
**Dev console — MIT live telemetry (#279 / 1d+1f, ADR 016 + ADR 017), 4 new import-light modules (TDD <1s, 24 tests):**
...
**Revert hazard (dev console):** these are additive seams — removing them drops `GET /status` + `/status/stream` (the Dashboard falls back to mock); they touch no translate path.
