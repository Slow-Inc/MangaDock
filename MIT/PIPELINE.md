...
**Dev console — MIT live telemetry (#279 / 1d+1f, ADR 016 + ADR 017), 7 new import-light modules (TDD <1s):**
...
- `auth.py` — `verify_supabase_token` via Supabase `GET /auth/v1/user` (httpx; **no PyJWT / no JWT secret in MIT**, mirrors the Backend's `getUser`; **refines ADR 016**'s PyJWT assumption — robust to the new asymmetric `sb_publishable_…` keys) + `is_staff` (staffLevel claim OR `MIT_STAFF_USER_IDS` allowlist). Fails closed. `test/test_auth.py` ×8.
