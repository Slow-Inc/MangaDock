...
## 2026-06-29 — MIT #180 step 3: wire kinsoku into the Knuth-Plass DP (pure, default-off)
Connected the two existing #180 pure modules. `line_break.find_optimal_line_breaks` gained `respect_kinsoku` (default **False** → byte-identical to the balance path, proven by a test) + a large-but-**finite** `kinsoku_penalty`: the break that opens a line is charged when it would strand a 行頭禁則 char at the line start or a 行末禁則 char at the previous line end. Finite (not a hard skip) so a forbidden break is avoided whenever a legal alternative exists yet a run where *every* break violates a rule still returns full in-order coverage instead of deadlocking. Co-located `kinsoku.py` at the package root (was `rendering/`) beside its only consumer `line_break.py`, so the breaker stays PIL/cv2-free (`rendering/__init__` pulls cv2+shapely) — satisfies the #180 "pure arithmetic, no ML/PIL imports" AC; only `test_kinsoku` referenced the old path (updated). TDD RED→GREEN, +4 tests (start-rule, end-rule, default-off byte-identity, no-deadlock). Also added **Latin hyphenation of an over-wide word** (`hyphenate_overwide_tokens`, commit `f2129af`): a pure opt-in pre-pass that splits a token wider than the column into fitting `-`-suffixed fragments (last has no hyphen), which the DP's existing `hyphen_penalty` then prices; a fitting word passes through untouched (byte-identical) and a too-narrow column degrades to one char/fragment so it always terminates with lossless coverage (+4 tests). **26 related tests green** (line_break 13 + kinsoku 9 + line_breaker 4). #180 **AC1's kinsoku + Latin-hyphenation bullets now done at the pure layer** (the even-balance bullet pre-existed in step 1); wiring into the bubble-fit narrow column (AC2) + the visual benchmark stay **blocked on #179 + a live MIT stack**. Branch `worktree-feat-mit-knuth-plass-kinsoku` (commits `8cf775f`/`f2129af`, extends PR #425). PIPELINE.md §5 + BENCHMARK.md come with the wiring step (runtime is unchanged while default-off + unwired).
