...
| **S26** | `BatchModeOrchestrator` (**last**; MemoryError ladder) | async-orch | high | ✅ S26a E2E byte-exact · S26b pending | **S26a** `70792af` — `batch_orchestration.py` pure helpers: `placeholder_context` (dedup 2 sites) + `build_page_translation_record` (L7 page records; 3 golden cases). **S26b** (MemoryError pre-process ladder → `_preprocess_image_for_batch`) deferred to a focused pass. Driver →**1934** |
