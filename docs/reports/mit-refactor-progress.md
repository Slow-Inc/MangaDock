...
| **S12** | `PipelineParams` + `apply_global_settings` | mixed | med | ✅ | `pipeline_params.py` — `apply_global_settings` (`_MODEL_DIR`+TF32) + `PipelineParams.from_params` value-object (device/using_gpu/gpu-limited/raise + batch_concurrent auto-disable + field parsing, byte-identical); `parse_init_params` delegates. 8 char tests (torch-availability mocked). **Closes #187** |
...
| **S23** | `StageRunner` (uniform progress + try/except policy) | async-orch | high | ✅ E2E byte-exact | `f1ce7a3` — `stage_runner.run_stage` + thin `_run_stage`; folded **8** sites in `_translate` + **5** in `_translate_until_translation`. Rendering kept **inline** (it reports `rendering` → conditional `rendering_folder:` BEFORE the run; `_run_stage` couples report+run). until/after method-split **deferred** (preload/save_input_png prefix + image_context suffix + early-exit returns diverge load-bearingly, à la S18). 5 cases |
| **S24** | `PatchRenderer` (extract `_process_group`; share.py:99 contract) | async-orch | med | ✅ E2E byte-exact | **S24a** `2eac7dd` — `patch_geometry.py` (3 self-free numpy fns: `build_local_region`/`create_text_only_mask`/`crop_mask_for_patch`; 8 golden cases). **S24b** `8fa69d3` — `patch_renderer.py` `PatchRenderer.process_group` (the ~90-line closure; `{x,y,w,h,img_png}` + all fallbacks byte-stable; 3 stub-orchestration cases). Driver: 2235→**1999** lines |
| **S22** | `DispatchRegistry` (fold get/cache/unload quintet across 6 dispatch modules) + kill global `MODEL` in detection | stateful | high | ✅ E2E byte-exact | S22a/b + global-MODEL (default/dbnet/craft); `test_dispatch_registry` (5) + `test_det_forward_default` (2); rebased onto main 342 pass / 0 new fail |
| **S25** | `PipelineOrchestrator` (drive stage-list; remove dead 'cancel') | async-orch | high | ✅ E2E byte-exact | `dfa0eb1` — `_run_until_translation_stages(ctx,config)->(ctx,finished)`: folds the ~80-line colorize→pre-dict block shared by `_translate` + `_translate_until_translation` (the dup S23 left). early-exit returns `(revert,True)` ⇒ caller `if finished: return ctx` = byte-identical. divergence stays at call sites; **L4 'cancel' preserved** (dead-code removal deferred). Cross-entry stage-list unify **not** done (load-bearing divergence). 3 cases |
| **S26** | `BatchModeOrchestrator` (**last**; MemoryError ladder) | async-orch | high | ✅ S26a+S26b done | **S26a** `70792af` — `batch_orchestration.py` pure helpers: `placeholder_context` (dedup 2 sites) + `build_page_translation_record` (L7 page records; 3 golden cases). **S26b** `35390fa` — `_preprocess_image_for_batch(image,config,i,mem_opt)->(ctx,cfg)`: the ~50-line try/MemoryError/Exception ladder out of the loop (loop keeps the psutil check + appends the return). byte-identical; 5 cases (success / MemoryError→recovery-config retry / retry-fail→placeholder / generic→placeholder / re-raise when mem-opt off). MIT rebooted on S26b → `/ready` 200 (boot-validated); single-page byte-exact path untouched |
...
| #187 | MangaTranslator god object → stage orchestrators | ✅ DONE — all S1-S26 seams landed (S12 value-object 2026-06-11 was the last) |
| #188 | model load/lifecycle + translator base abstractions | model-lifecycle half DONE (S3/S4/S20/S21/S22 + global `MODEL` killed in detection); translator base-abstraction half (BaseGPTTranslator) still open (xhigh) |
