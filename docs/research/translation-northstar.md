...
|---|---|---|
| **เห็นภาพ ไม่ใช่แค่ข้อความ** (เข้าใจฉาก/อารมณ์/ผู้พูด) | ส่งแต่ text ให้ LLM | **PRD #171 P1** (multimodal OCR) |
...
| **คงการเน้น** (ตัวหนา/เอียง/ตะโกน) | ทิ้ง emphasis หมด | **PRD #171 P2** (emphasis markers) |
| **ตรวจทานรอบสอง** (QA จับ hallucination/ผิดบริบท) | post-translation check (ตัวซ้ำ+สัดส่วนภาษา) เปิดอยู่ | มีบางส่วน — semantic = **PRD #171 P3** |
| **honorific/วัฒนธรรม** (-kun→-คุง, นาย/คุณ) | LLM เดาเอง | glossary (#160) หรือ instruction ใน gpt_config |

## 3 PRD ที่ขับ north star นี้
...
มิติที่เคยยังไม่มี PRD → ตอนนี้ครอบด้วย **PRD #171** (read & write like a human): multimodal OCR, emphasis markers, semantic proofread
...
|---|---|---|
| Sees the art, not just text (scene, mood, speaker) | text-only to LLM | **PRD #171 P1** (multimodal OCR) |
...
| Preserves emphasis (bold/italic/shout) | emphasis dropped | **PRD #171 P2** (emphasis markers) |
| Proofreads (catch hallucination/context errors) | repetition + lang-ratio check ON | partial — semantic = **PRD #171 P3** |
| Honorifics/cultural register | LLM guesses | glossary (#160) / gpt_config instruction |

## The three PRDs driving this
...
Those three formerly-uncovered dimensions are now **PRD #171** (read & write like a human): multimodal OCR, emphasis markers, semantic proofread.
